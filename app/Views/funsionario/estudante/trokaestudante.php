...
<?= $this->section('funsionario') ?>
    <title><?=lang('sidebarPortfolio.trokaEstudantePortfolio')?></title>
<?= $this->endSection() ?>
<?= $this->section('funsionario') ?>
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title"><strong><?=lang('sidebarPortfolio.trokaEstudantePortfolio')?></strong></h4>
            </div>
            <?php $errors = session()->getFlashdata('errors'); ?>
            <div class="card-body">
                <div class="basic-form">
                    <form action="<?=base_url(lang('registuEstudanteFunsionario.updateEstudanteRegistoUrlPortfolio').$student->id_estudante_registo) ?>" method="post">
                            <?= csrf_field(); ?>
                            <div class="row g-2">
                                <div class="mb-3 col-md-6">
                                    <label for="inputState" class="form-label"><?=lang('funsionarioPortfolio.contaEstudante')?></label>
                                    <select id="inputState" name="conta_estudante_registo" class="form-control">
                                        <option value="<?=$student->conta_estudante_registo?>"><?=$student->naran_kompleto?>(<sub><?=$student->sistema?></sub>)</option>
                                        <option value=""><?=lang('funsionarioPortfolio.hiliContaEstudante')?></option>
                                        <?php foreach($estudante as $pro): ?>
                                        <option value="<?=$pro->id_estudante?>"<?= old('conta_estudante_registo') == $pro->id_estudante ? 'selected' : null ?>><?=$pro->naran_kompleto?>(<sub><?=$pro->sistema?></sub>)</option>
                                        <?php endforeach; ?>
                                    </select>
                                    <small class="text-danger">
                                       <?=isset($errors['conta_estudante_registo']) ?  $errors['conta_estudante_registo'] : null ?>
                                    </small>
                                </div>
                                <div class="mb-3 col-md-6">
                                    <label for="inputEmail4" class="form-label"><?=lang('funsionarioPortfolio.tituloFunsionario')?></label>
                                    <input type="date" name="data_estudante_registo" value="<?=old('data_estudante_registo', $student->data_estudante_registo) ?>" class="form-control" id="inputEmail4" placeholder="<?=lang('funsionarioPortfolio.tituloFunsionario')?>">
                                    <small class="text-danger">
                                       <?=isset($errors['data_estudante_registo']) ?  $errors['data_estudante_registo'] : null ?>
                                    </small>
                                </div>
                            </div>
                            <button type="submit" class="btn btn-primary"><i class="mdi mdi-content-save-outline"></i></button>
                            <a href="<?= base_url(lang('registuEstudanteFunsionario.estudanteRegistoUrlPortofolio'))?>" class="btn btn-info ms-1"><i class="mdi mdi-skip-previous"></i></a>
                        </form> 
                </div>
            </div>
        </div>
