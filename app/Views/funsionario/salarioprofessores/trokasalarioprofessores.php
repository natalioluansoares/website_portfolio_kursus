...
<?= $this->section('funsionario') ?>
    <title><?=lang('sidebarPortfolio.trokaSalarioProfessoresPortfolio')?></title>
<?= $this->endSection() ?>
<?= $this->section('funsionario') ?>
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title"><strong><?=lang('sidebarPortfolio.trokaSalarioProfessoresPortfolio')?></strong></h4>
            </div>
            <?php $errors = session()->getFlashdata('errors'); ?>
            <div class="card-body">
                <div class="basic-form">
                    <form action="<?=base_url(lang('salarioProfessores.updateSalarioProfessoresUrlPortfolio').$salario->id_salario_professores) ?>" method="post">
                        <?= csrf_field(); ?>
                        <div class="row g-2">
                            <div class="mb-3 col-md-6">
                                ...
                                <select id="inputState" name="salario_professores" class="form-control">
                                    <?php if ($salario->sistema == 'Professores') { ?>
                                        <option value="<?=$salario->salario_professores?>"><?=$salario->naran_kompleto?>. <?=$salario->titulo_professores?>(<sub><?=lang('salarioProfessores.professoresFunsionario')?></sub>)</option>
                                   <?php }?>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input type="number" name="total_salario" min="0" value="<?=old('total_salario', $salario->total_salario) ?>" class="form-control" id="inputEmail4" placeholder="<?=lang('salarioProfessores.totalFunsionario')?>">
                                <small class="text-danger">
                        ...
                        <a href="<?= base_url(lang('salarioProfessores.salarioFunsionarioUrlPortofolio'))?>" class="btn btn-info ms-1"><i class="mdi mdi-skip-previous"></i></a>
                    </form> 
                </div>
            </div>
        </div>
