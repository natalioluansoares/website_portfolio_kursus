...
<?= $this->section('funsionario') ?>
    <title><?=lang('sidebarPortfolio.aumentaMateriaProfessoresPortfolio')?></title>
<?= $this->endSection() ?>
<?= $this->section('funsionario') ?>
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title"><?=lang('sidebarPortfolio.aumentaMateriaProfessoresPortfolio')?></strong></h4>
            </div>
            <?php $errors = session()->getFlashdata('errors'); ?>
            <div class="card-body">
                <div class="basic-form">
                    <form action="<?=base_url(lang('materiaProfessores.inputMateriaProfessoresUrlPortfolio')) ?>" method="post">
                        <?= csrf_field(); ?>
                        <div class="row g-2">
                            <div class="mb-3 col-md-6">
                                <label for="inputEmail4" class="form-label"><?=lang('materiaProfessores.kodeMateria')?></label>
                                <input type="text" name="kode_materia_professores" value="<?=old('kode_materia_professores') ?>" class="form-control" id="inputEmail4" placeholder="<?=lang('materiaProfessores.kodeMateria')?>">
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select id="inputState" name="materia_professores" class="form-control">
                                    ...
                                    <option value="<?=$pro->id_professores?>"<?= old('materia_professores') == $pro->id_professores ? 'selected' : null ?>><?=$pro->naran_kompleto?>(<sub><?=$pro->sistema?></sub>)</option>
                                    <?php endforeach; ?>
                                </select>
                                <small class="text-danger">
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input type="text" name="data_materia_professores" value="<?=old('data_materia_professores') ?>" class="form-control" id="mdate" placeholder="<?=date('Y-M-d')?>">
                                <small class="text-danger">
