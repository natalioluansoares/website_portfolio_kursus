...
class Accountportfolio extends ResourceController
{
    ...
    public function index()
    {
        ...
        // print_r($keyword);
         $data = $this->administrator->orderBy('id_administrator', 'DESC')->where('aktivo_administrator =', null)->where('sistema_administrator =', '1')
         ->administratorPagination(10, $keyword);
            
         $data ['keyword']= $keyword;

         if (null !== $this->request->getGet('filter-data')) {
            $data = $this->administrator->where('aktivo_administrator =', null)->where('sistema_administrator =', '1')->filteradministrator($keyword);
            if ($data['administrator'] == null) {
                return redirect()->to(base_url(lang('registoAdministrator.registoUrlConta')))->with('error', ''.lang('registoAdministrator.bukaValidation').'');
            }
         }else{
            $keyword = $this->request->getGet('keyword');
            // print_r($keyword);
            $data = $this->administrator->orderBy('id_administrator', 'DESC')->where('aktivo_administrator =', null)->where('sistema_administrator =', '1')->administratorPagination(10, $keyword);
            $data ['keyword']= $keyword;
         }
        return view('administrator/registoadministrator/registo', $data);
    }
    public function hamos()
    {
        ...
        $keyword = $this->request->getGet('keyword');
        // print_r($keyword);
         $data = $this->administrator->orderBy('id_administrator', 'DESC')->where('aktivo_administrator =', 1)->where('sistema_administrator  =', 1)
         ->administratorPagination(10, $keyword);
            
         $data ['keyword']= $keyword;

         if (null !== $this->request->getGet('filter-data')) {
            $data = $this->administrator->where('aktivo_administrator =', 1)->where('sistema_administrator  =', 1)->filteradministrator($keyword);
            if ($data['administrator'] == null) {
                return redirect()->to(base_url(lang('registoAdministrator.hamosRegistoUrlConta')))->with('error', ''.lang('registoAdministrator.bukaValidation').'');
            }
         }else{
            $keyword = $this->request->getGet('keyword');
            // print_r($keyword);
            $data = $this->administrator->orderBy('id_administrator', 'DESC')->where('aktivo_administrator =', 1)->where('sistema_administrator  =', 1)->administratorPagination(10, $keyword);
            $data ['keyword']= $keyword;
         }
        return view('administrator/registoadministrator/hamosregisto', $data);
    }
    ...
    public function new()
    {
        $data ['sistema'] = $this->sistema->where('aktivo_sistema =', null)->where('id_sistema =', 1)->findAll();
        return view('administrator/registoadministrator/aumentaregisto', $data);
    }
    ...
    public function create()
    {
        $validate =$this->validate([
            ...
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.ikusValidation').'',
                ],
            ],
            'naran_primeiro' => [
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.primeiroValidation').'',
                ],
            ],
            'naran_primeiro' => [
            'rules'=>'required',
            ...
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.kompletoValidation').'',
                ],
            ],
            'email' => [
            'rules'=>'required|valid_email',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.emailValidation').'',
            ...
            'errors' => [
                'required'  => ''.lang('registoAdministrator.passwordValidation').'',
            ...
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.jeneroValidation').'',
                ],
            ],
            'fatin_moris' => [
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.fatinValidation').'',
                ],
            ],
            'loron_moris' => [
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.loronValidation').'',
                ],
            ],
            'status_servisu' => [
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.statusValidation').'',
                ],
            ],
            'sistema_administrator' => [
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.sistemaValidation').'',
                ],
            ],
            'numero_whatsapp' => [
            'rules'=>'required',
            'errors' => [
                'required'  => ''.lang('registoAdministrator.whatsappValidation').'',
                ],
            ],
            'numero_eleitural' => [
        ...
        if (!$validate) {
            ...
            if ($this->administrator->cek_naran($naran_kompleto)->resultID->num_rows > 0) {
                return redirect()->back()->with('error', ''.lang('registoAdministrator.ihaKompletoValidation').'');
            }elseif ($email == null) {
                return redirect()->back()->withInput()->with('error', ''.lang('registoAdministrator.emailValidation').'');
            }elseif ($this->administrator->cek_email($email)->resultID->num_rows > 0) {
                return redirect()->back()->with('error',''.lang('registoAdministrator.ihaEMailValidation').'');
            }else {
                $data = [
               ...
               if ($insert) {
                ...
                if (!$insert) {
                    return redirect()->back()->withInput()->with('error', ''.lang('registoAdministrator.errorValidation').'');
                }else{
                    $image->move('uploads/fotoportfolio/', $datafoto);
                    return redirect()->to(base_url(lang('registoAdministrator.registoUrlConta')))->with('success',''.lang('registoAdministrator.successValidation').'');
                }
            }
        }
    }
    public function image($id=null)
    {
        ...
        if (!$data['administrator']) {
            return redirect()->back()->withInput()->with('error', ''.lang('registoAdministrator.bukaValidation').'');
        }
        return view('administrator/registoadministrator/trokafoto', $data);
    }
    ...
    public function edit($id = null)
    {
        ...
        return view('administrator/registoadministrator/trokaregisto', $data);
    }
    ...
    public function temporario($id = null)
    {
        ...
        if ($this->db->affectedRows() > 0) {
            return redirect()->to(site_url(lang('registoAdministrator.hamosRegistoUrlConta')))->with('success','Troka Fila Fali Tiha Ona Dados Temporario Sai Hanesan Dados Permanente');
        }else {
            return redirect()->to(site_url(lang('registoAdministrator.hamosRegistoUrlConta')))->with('error','Troka Dados Temporario Ba Dados Permanente Seidauk Iha');
            # code...
        }
    }
    ...
    public function permanente($id = null){
        ...
        return redirect()->back()->with('success', ''.lang('registoAdministrator.hamosValidation').'');
    }
}
