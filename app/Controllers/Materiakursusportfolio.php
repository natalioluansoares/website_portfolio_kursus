...
class Materiakursusportfolio extends ResourceController
{
    protected $kursus;
    protected $db;
    ...
    public function index()
    {
        ...
        if (null !== $this->request->getGet('filter-data')) {
            ...
            if ($data['materiakursus'] == null) {
                return redirect()->to(base_url(lang('materiaKursusFunsionario.materiaKursussUrlPortfolio')))->with('error', '' . lang('professoresPortfolio.bukaValidation') . '');
            }
        } else {
            $keyword = $this->request->getGet('keyword');
            // print_r($keyword);
        ...
        return view('administrator/materiakursus/materiakursus', $data);
    }
    ...
    public function new()
    {
        return view('administrator/materiakursus/aumentamateriakursus');
    }
    ...
    public function create()
    {
        ...
        if (!$validate) {
            ...
            if (!$insert) {
                ...
                return redirect()->to(base_url(lang('materiaKursusFunsionario.materiaKursussUrlPortfolio')))->with('success', '' . lang('materiaKursusFunsionario.successValidation') . '');
            }
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        return view('administrator/materiakursus/trokamateriakursus', $data);
    }
    ...
    public function hamosmateriakursus()
    {
        $keyword = $this->request->getGet('keyword');
        // print_r($keyword);
        $data = $this->kursus->orderBy('id_materia_kursus', 'DESC')->where('aktivo_materia_kursus =', '1')->materiaKursusPagination(10, $keyword);

        $data['keyword'] = $keyword;

        if (null !== $this->request->getGet('filter-data')) {
            $data = $this->kursus->where('aktivo_materia_kursus =', '1')->filterMateriaKursus($keyword);
            if ($data['materiakursus'] == null) {
                return redirect()->to(base_url(lang('materiaKursusFunsionario.materiaKursussUrlPortfolio')))->with('error', '' . lang('professoresPortfolio.bukaValidation') . '');
            }
        } else {
        ...
        return view('administrator/materiakursus/hamosmateriakursus', $data);
    }

    public function hamoshotutemporario()
    {
        $id = $this->request->getPost('id');
        $number = count($id);
        ...
        for ($i = 0; $i < $number; $i++) {
            $data = [
                'id_materia_kursus' =>$id[$i],
                'aktivo_materia_kursus' =>$aktivo_materia_kursus[$i]

            ];
            $update = $this->db->table('materia_kursus')->where('id_materia_kursus', $id[$i])->update($data);
        }
        if (!$update) {
            return redirect()->back()->withInput()->with('error', '' . lang('materiaPortfolio.errorValidation') . '');
        } else {
    ...
    public function hamostemporario()
    {
        $id = $this->request->getPost('id');
        $aktivo_materia_kursus = $this->request->getPost('aktivo_materia_kursus');
        ...
        $data = [
            ...
            $update = $this->db->table('materia_kursus')->where('id_materia_kursus', $id)->update($data);
        if (!$update) {
            return redirect()->back()->withInput()->with('error', '' . lang('materiaPortfolio.errorValidation') . '');
        } else {
            return redirect()->back()->withInput()->with('success', '' . lang('materiaPortfolio.hamosValidation'));
    ...
    {
        $id = $this->request->getPost('id');
        $update = $this->db->table('materia_kursus')->where('id_materia_kursus', $id)->delete();
        if (!$update) {
            return redirect()->back()->withInput()->with('error', '' . lang('materiaPortfolio.errorValidation') . '');
        } else {
            return redirect()->back()->withInput()->with('success', '' . lang('materiaPortfolio.hamosValidation'));
    ...
    {
        $data = $this->kursus->where('aktivo_materia_kursus =', '1')->findAll();
         foreach($data as $hamos){
             $update = $this->db->table('materia_kursus')->where('id_materia_kursus', $hamos->id_materia_kursus)->delete();
         }
        if (!$update) {
            return redirect()->back()->withInput()->with('error', '' . lang('materiaPortfolio.errorValidation') . '');
        } else {
